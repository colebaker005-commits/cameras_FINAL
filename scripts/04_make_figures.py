...
Figure 2 — Placebo comparison: horizontal bar chart showing the real DiD
    vs the placebo DiD for each camera type. If the real bars look very
    different from the placebo bars, the effect is genuine.

Figure 3 — Pre vs post scatter: one dot per camera, x = crashes before
...
Output:
    ...
    outputs/figures/fig_placebo.png
...
def style_axes(ax):
    """Apply shared editorial style to an axes."""
...
def compute_did(sub, nearby_pre, nearby_post, city_pre, city_post):
    """Compute DiD for a subset of cameras given column names."""
...
def fig_map(crashes, res):
    ...
    # --- Diverging colormap: green = improvement, red = worse.
...
def fig_placebo(res, placebo):
    ...
    for t in TYPES_ORDER:
        real_dids.append(compute_did(
            res[res["type"] == t],
            "nearby_injury_pre", "nearby_injury_post",
            "citywide_injury_pre", "citywide_injury_post"))
        placebo_dids.append(compute_did(
            placebo[placebo["type"] == t],
            "placebo_nearby_injury_pre", "placebo_nearby_injury_post",
            "placebo_city_injury_pre",   "placebo_city_injury_post"))

    y = np.arange(len(TYPES_ORDER))
    h = 0.36
    b1 = ax.barh(y - h / 2, real_dids,    h, color=NAVY,     edgecolor=INK,
                 linewidth=0.5, label="Real install date")
    b2 = ax.barh(y + h / 2, placebo_dids, h, color="#BFB7A4", edgecolor=INK,
                 linewidth=0.5, label="Placebo (1 year earlier)")
...
def fig_scatter(res):
    """Figure 3 — pre vs post injury crashes, one dot per camera, by type."""
    ...
    for ax, t in zip(axes, TYPES_ORDER):
        ...
        # --- The diagonal line = no change. Dots below it = fewer crashes after.
...
def main():
    ...
    # placebo already has a 'type' column from script 04 — no merge needed.
    ...
    # --- Small summary table of the DiD values shown in the figures.
    print("\nWriting DiD summary xlsx ...")
    rows = []
    for t in TYPES_ORDER:
        rows.append({
            "camera_type":        t,
            "n":                  len(res[res["type"] == t]),
            "real_did_injury":    round(compute_did(
                res[res["type"] == t],
                "nearby_injury_pre", "nearby_injury_post",
                "citywide_injury_pre", "citywide_injury_post"), 2),
            "placebo_did_injury": round(compute_did(
                placebo[placebo["type"] == t],
                "placebo_nearby_injury_pre", "placebo_nearby_injury_post",
                "placebo_city_injury_pre",   "placebo_city_injury_post"), 2),
        })
